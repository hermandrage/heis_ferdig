...
void run_states(void){
  ...
  while (1){
      ...
      switch (current_state){
        ...
        if (elev_get_floor_sensor_signal()!=-1){ //sets current_direction to DIRN_STOP if not yet set.
          set_current_state(DOOR_OPEN);

        }
        else if(read_next_order()!=-1){//is activatid if there is an order in que
          if(read_next_order()==current_foor){
            ...
            if(read_next_order()-current_foor>0){
              set_current_state(DRIVE_UP);
            }
            if(read_next_order()-current_foor<0){
              set_current_state(DRIVE_DOWN);
